...
async function processWithPython(req: Request, res: Response) {
  try {
    ...
    const cleanedAnswerKey = finalAnswerKey.filter((answer: string) => answer && answer.trim() !== '')
    ...
    console.log('📝 Final answer key being used:', cleanedAnswerKey)
...
async function processWithNodeJS(req: Request, res: Response) {
  try {
    ...
    // Get exam metadata and prioritize answerKey from exam-keys page
    let examMetadata = null
    let finalAnswerKey = answerKey
